    ...
    document.querySelectorAll('[data-marks]').forEach(marksInput => {
        ...
        marksInput.addEventListener('input', function () {
            ...
            if (!isNaN(val) && val >= 0 && val <= total) {
                percField.value = ((val / total) * 100).toFixed(2) + '%';
            } else {
                percField.value = '';
            }
        });
...
function initHscCalc() {
    ...
    function calcPcm() {
        ...
        if (pcmPercField) {
            pcmPercField.value = pcmSum > 0
                ? ((pcmSum / 300) * 100).toFixed(2) + '%'
                : '';
        }
    }
    ...
    if (hscMarksField && hscPercField) {
        hscMarksField.addEventListener('input', function () {
            const val = parseFloat(this.value);
            hscPercField.value = (!isNaN(val) && val >= 0 && val <= 600)
                ? ((val / 600) * 100).toFixed(2) + '%'
                : '';
        });
    }
}
...
async function submitPG() {
    ...
    const data = {
        ...
        degree_max: parseInt(g('pg-degree-max')) || null,
        degree_obtained: parseInt(g('pg-degree-obtained')) || null,
...
document.addEventListener('DOMContentLoaded', () => {
    initHamburger();
    populateSchoolDropdowns(); // fill all school selects with Pondicherry groups
    initAgeCalc();
    initCapsFields();
    initPercentCalc();
    initHscCalc();
    initToggleGroups();
    initSchoolDropdown();     // wire "Other" reveal listeners
    initPrint();
